...
fn solve() {
    ...
    let mut sum_d = vec![vec![vec![0u64; r + 1]; c + 1]; r + 1];
    let mut sum_r = vec![vec![vec![0u64; c + 1]; c + 1]; r + 1];
    for i in 1..r + 1 {
        for j in 1..c + 1 {
            sum_d[i][j][i - 1] = 0;
    ...
    // [i,j,k], [k,0]- [i,j]
    let mut cum_d = vec![vec![vec![0u64; r+1]; c+1]; r+1];
    for j in 1..c+1{
        for i in 1..r+1 {
            for x in 1..i+1 {
                let mut t = 0u64;
    ...
    // [i,j,k]  [0,k]-[i,j]
    let mut cum_r = vec![vec![vec![0u64; c+1]; c+1]; r+1];
    for i in 1..r+1 {
        for j in 1..c+1 {
            for y in 1..j+1 {
                let mut t = 0u64;
